...
FaustSinOsc = function (myThread) {
  let ugens = "uwta["+myWorker+"]["+myThread.threadNum+"]"
  postMessage(ugens+".push(makeFaustSinOsc())") 
  return { dsp:myThread.dsp(), now:myThread.now+cushion,
    freq: function (hz) {
        let n = this.dsp
        postMessage(ugens+"["+n+"].osc.setParamValue(\"/Oscillator/freq\","+hz+")")
      },
    vol: function (db) {
        let n = this.dsp
        postMessage(ugens+"["+n+"].osc.setParamValue(\"/Oscillator/volume\","+db+")")
      },
