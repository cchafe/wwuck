// worker220a.js
// loaded by webAudio220a.js
// provides spork which can be used e.g. by calls like
//       spork(sonify,data)
// in hw1Sonification.js
...
function TimeIterator(n) {
  ...
  postMessage("gwta["+myWorker+"].push(new Array)") 
...
//console.log("this.now =",this.now,"ds =",ds)
  ...
  this.graphCnt = 0
  this.graph = function () {
    let graphNext = this.graphCnt
    this.graphCnt++
    return graphNext
  }
}

Graph = function (myThread) {
  let graphs = "gwta["+myWorker+"]["+myThread.threadNum+"]"
  postMessage(graphs+".push(makeGraph())") 
console.log(graphs)
console.log("2")
  return { graph:myThread.graph(), now:myThread.now+cushion,
    val: function (i,v) {
        let n = this.graph
        postMessage(graphs+"["+n+"].xy.push(["+i+","+v+"])")
//        postMessage("console.log(" + graphs+"["+n+"].xy.length)")
      },
    plot: function () {
      let n = this.graph;
      postMessage(graphs+"["+n+"].plot()")
    }
  }
}
